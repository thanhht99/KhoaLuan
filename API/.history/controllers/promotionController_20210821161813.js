...
exports.createNewPromotion = asyncMiddleware(async(req, res, next) => {
    ...
    const convertProducts = products.map((val) => {
        return { productSku: val };
    });
    console.log("🚀 ~ file: promotionController.js ~ line 68 ~ convertProducts ~ convertProducts", convertProducts)

    const checkProduct = convertProducts.filter((val) => {
        console.log("🚀 ~ file: promotionController.js ~ line 66 ~ checkProduct ~ val", val.productSku);
        console.log("🚀 ~ file: TEST", typeof(val));
        const check = Product.findOne({ sku: val.productSku });
    ...
    const promotion = new Promotion({ products: convertProducts, promotion_name, promotion_desc, discount, type, startDate: convertStartDate, endDate: convertEndDate });
