...
exports.createNewPromotion = asyncMiddleware(async(req, res, next) => {
    ...
    console.log("🚀 ~ file: promotionController.js ~ line 68 ~ convertProducts ~ convertProducts", objProducts);

    const findProduct = await Promise.all(objProducts.map(async(val) => {
        const a = await Product.findOne({ sku: val.productSku });
    }));
    console.log("🚀 ~ file: promotionController.js ~ line 71 ~ findProduct ~ findProduct", findProduct)

    // const checkProduct = convertProducts.filter(async(val) => {
    //     console.log("🚀 ~ file: promotionController.js ~ line 66 ~ checkProduct ~ val", val.productSku);
    //     const check = await Product.findOne({ sku: val.productSku });
    //     return check.sku === val.productSku;
    ...
    const promotion = new Promotion({ products: objProducts, promotion_name, promotion_desc, discount, type, startDate: convertStartDate, endDate: convertEndDate });
