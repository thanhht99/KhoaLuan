...
exports.createNewPromotion = asyncMiddleware(async(req, res, next) => {
    ...
    req.checkBody("startDate", "Start Date must be in correct format YYYY-MM-DDTHH:MM:SS !!").isISO8601().toDate();
    req.checkBody("endDate", "End Date must be in correct format YYYY-MM-DDTHH:MM:SS !!").isISO8601().toDate();
    ...
    console.log("🚀 ~ file: promotionController.js ~ line 68 ~ exports.createNewPromotion=asyncMiddleware ~ startDate >= endDate", startDate >= endDate)

    if (convertStartDate >= convertEndDate) {
        console.log("🚀 ~ file: promotionController.js ~ line 69 ~ exports.createNewPromotion=asyncMiddleware ~ convertStartDate >= convertEndDate", convertStartDate >= convertEndDate)
    ...
    const promotion = new Promotion({ products, promotion_name, promotion_desc, discount, type, startDate, endDate });
