...
exports.createNewPromotion = asyncMiddleware(async(req, res, next) => {
    ...
    const checkProduct = products.filter(async(val) => {
        console.log("🚀 ~ file: promotionController.js ~ line 66 ~ checkProduct ~ product", val)
        const check = await Product.find({ sku: val }).select('-updatedAt -createdAt -__v');
        console.log("🚀 ~ file: promotionController.js ~ line 68 ~ checkProduct ~ check", check)
        return check.sku === val;
        console.log("🚀 ~ file: promotionController.js ~ line 70 ~ checkProduct ~ check.sku === val", check.sku === val)
    });
